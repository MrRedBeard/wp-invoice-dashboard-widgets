...
class WPI_DW_Invoice_Statistics extends WPI_Dashboard_Widget {
  ...
  function config() {
    ...
    ?>
      ...
      <div class="table table_settings">
        ...
        <p class="sub"><?php _e( 'Goals', WPI ); ?></p>

        <table>
          <tr>
            <td>
              <?php echo WPI_UI::checkbox("name=opts[enable_goals]&value=true&label=" . __("Show Goal lines", WPI), WPI_Functions::is_true($widget_options['enable_goals'])); ?>
            </td>
          </tr>
          <tr>
            <td>
              <label><?php _e( 'Day Goal', WPI ); ?></label>
              <input type="text" value="<?php echo (float)$widget_options['day_goal']; ?>" name="opts[day_goal]" />
            </td>
          </tr>
          <tr>
            <td>
              <label><?php _e( 'Week Goal', WPI ); ?></label>
              <input type="text" value="<?php echo (float)$widget_options['week_goal']; ?>" name="opts[week_goal]" />
            </td>
          </tr>
          <tr>
            <td>
              <label><?php _e( 'Month Goal', WPI ); ?></label>
              <input type="text" value="<?php echo (float)$widget_options['month_goal']; ?>" name="opts[month_goal]" />
            </td>
          </tr>
        </table>
  ...
  function ajax_handler() {
    ...
    $options = WPI_DW_User_Invoices::get_dashboard_widget_options( 'wpi_dw_invoice_statistics' );
    ...
    $last_period = $wpdb->get_results( "SELECT * FROM {$wpdb->base_prefix}wpi_object_log WHERE time > {$stamp} AND ( action = 'add_payment' OR action = 'refund' ) ORDER BY time", ARRAY_A );

    $data = array();
    $goals = array('enable'=>false);
    ...
    if ( $options['enable_goals'] == 'true' ) {
      $goals['enable'] = true;
      $goals['day'] = (float)$options['day_goal'];
      $goals['week'] = (float)$options['week_goal'];
      $goals['month'] = (float)$options['month_goal'];
    }

    if ( !empty( $data ) ) {
      die( json_encode( array( 'success' => true, 'data' => array('labels' => $labels, 'values' => $values), 'goals' => $goals ) ) );
    }
